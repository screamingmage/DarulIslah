...
k.appendChild(l);k.setAttribute("aria-label",b);f[a]=k}else f[a].textContent=b}q?(b="",p&&(b+='<div class="user">'+v(g[a].innerHTML)+"</div>"),n&&(b+='<p><a class="timePosted" href="#">'+f[a].getAttribute("aria-label")+"</a>"),b+=''+v(e[a].innerHTML)+"</p>"):e[a].innerText?(b="",p&&(b+='<p class="user">'+g[a].innerText+"</p>"),n&&(b+='<p><a href="#" class="timePosted">'+f[a].innerText+"</a>"),b+=''+e[a].innerText+"</p>"):(b="",p&&(b+='<p class="user">'+g[a].textContent+"</p>"),n&&(b+='<p><a class="timePosted">'+f[a].textContent+"</a>"),b+=''+
e[a].textContent+"</p>");c.push(b);a++}if(null==u){e=c.length;g=0;f=document.getElementById(w);for(d="<ul>";g<e;)d+="<li>"+c[g]+"</li>",g++;f.innerHTML=d+"</ul>"}else u(c);r=!1;0<h.length&&(twitterFetcher.fetch(h[0].id,h[0].domId,h[0].maxTweets,h[0].enableLinks,h[0].showUser,h[0].showTime,h[0].dateFunction,h[0].showRt,h[0].customCallback),h.splice(0,1))}}}();
